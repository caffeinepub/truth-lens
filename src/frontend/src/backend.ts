...
export class Backend implements backendInterface {
    ...
    async readConfigValue(arg0: string): Promise<string | null> {
        if (this.processError) {
            try {
    ...
    async saveConfigValue(arg0: string, arg1: string): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.saveConfigValue(arg0, arg1);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
