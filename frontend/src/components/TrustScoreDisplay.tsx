...
import { Shield, ShieldAlert, CheckCircle2, AlertTriangle, XCircle, Globe, Scan, Fish } from 'lucide-react';
...
function APISourceCard({ source }: { source: APISourceResult }) {
  ...
  const statusBorder = source.isSafe ? 'border-accent/20' : 'border-destructive/20';

  return (
    <div className={`flex items-start gap-3 p-4 rounded-lg border ${statusBorder} ${statusBg}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className={`text-xs font-medium ${statusColor}`}>{source.confidence}%</span>
        </div>
      </div>
    </div>
  );
...
export default function TrustScoreDisplay({ isSafe, confidence, apiSources }: TrustScoreDisplayProps) {
  ...
  return (
    <Card className={`${statusBorder} border-2 shadow-lg bg-card/50 backdrop-blur-sm`}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-2xl">Trust Score Analysis</CardTitle>
          <Badge variant={isSafe ? 'default' : 'destructive'} className="text-lg px-4 py-2">
            {isSafe ? 'SAFE' : 'UNSAFE'}
          </Badge>
        </div>
        <CardDescription>Comprehensive threat assessment from 3 security APIs</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Status Icon and Message */}
        <div className={`flex items-center gap-4 p-6 rounded-lg ${statusBg} border ${statusBorder}`}>
          {isSafe ? (
            <Shield className={`h-16 w-16 ${statusColor} ${glowClass} shrink-0`} />
          ) : (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <Label className="text-lg font-semibold">Overall Confidence Level</Label>
            <span className={`text-2xl font-bold ${statusColor} ${glowClass}`}>{confidence}%</span>
          </div>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            {confidence >= 90 ? (
              ...
              <>
                ...
                <span>Lower confidence — additional verification recommended</span>
              </>
            )}
          </div>
        </div>
