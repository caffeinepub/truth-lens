...
const CONFIG_FIELDS: ConfigField[] = [
  ...
  },
  {
    ...
    description: 'Set to "true" to enable maintenance mode (disables scanning for regular users).',
    placeholder: 'false',
  },
  {
    key: 'welcomeMessage',
...
export default function SiteConfigurationTab() {
  ...
  const handleSave = async () => {
    ...
    if (successCount > 0) {
      toast.success(`${successCount} setting${successCount > 1 ? 's' : ''} saved successfully`);
      setDirty(new Set());
    }
    if (errorCount > 0) {
      toast.error(`Failed to save ${errorCount} setting${errorCount > 1 ? 's' : ''}`);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Configure global application settings. Changes take effect immediately.
        </p>
        <Button
          onClick={handleSave}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {CONFIG_FIELDS.map((field) => (
          <Card key={field.key} className="cyber-border bg-card/50 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                <Settings className="h-4 w-4 text-primary" />
                <CardTitle className="text-sm font-semibold">{field.label}</CardTitle>
                {dirty.has(field.key) && (
                  <span className="text-xs text-yellow-400 ml-auto">● Modified</span>
                )}
              </div>
