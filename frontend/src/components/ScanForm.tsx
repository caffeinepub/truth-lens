  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    scanContent(input, {
      onSuccess: (data) => {
        ...
        sessionStorage.setItem('lastScanResult', JSON.stringify({ result: data, input }));
        navigate({ to: '/results' });
      },
    });
  };

  const isFormValid = activeTab === 'url' ? urlInput.trim().length > 0 : textInput.trim().length > 0;
  ...
  return (
    <Card className="cyber-border shadow-cyber-glow bg-card/50 backdrop-blur-sm">
      <CardHeader>
        ...
        <CardDescription>Choose your input method and scan for potential threats</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <Alert variant="destructive" className="mt-4">
              ...
              <AlertDescription>{error.message || 'Failed to scan content. Please try again.'}</AlertDescription>
            </Alert>
          )}
